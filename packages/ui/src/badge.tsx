...
export function Badge({ tone = "neutral", dot, className, children, ...props }: BadgeProps) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-2 whitespace-nowrap rounded-[var(--radius-pill)] px-2 py-1 text-[var(--text-xs)] font-[var(--weight-semibold)] transition-colors duration-150 ease-out",
        tones[tone],
        className,
      )}
      {...props}
