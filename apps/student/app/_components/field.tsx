  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className={cn(
          "h-[var(--input-height)] w-full scroll-mt-20 rounded-[var(--radius-md)] border bg-[var(--color-surface)] px-3.5 text-[var(--text-base)] text-[var(--color-text-primary)] outline-none transition-[border-color,box-shadow] placeholder:text-[var(--color-text-tertiary)] focus:border-[var(--color-border-focus)] focus:shadow-[var(--focus-ring)] read-only:bg-[var(--color-background-subtle)] read-only:text-[var(--color-text-secondary)]",
          error
